...
function BuchenFormular() {
  ...
  const absenden = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.fahrzeug) { setFehler('Bitte ein Fahrzeug auswaehlen.'); return; }
    if (!form.pilot) { setFehler('Bitte einen Piloten auswaehlen.'); return; }
      ...
      <form onSubmit={absenden} className="bg-white rounded-xl shadow p-6 space-y-4">
            ...
            {PILOTEN.map((p) => {
              ...
              return (
                <label key={p} className={[
                  ...
                  form.pilot === p ? `border-current ${pf.bg} ${pf.text}` : 'border-gray-200 hover:border-gray-300',
                ].join(' ')}>
                  <input type="radio" name="pilot" value={p}
                    checked={form.pilot === p}
                    onChange={(e) => setForm({ ...form, pilot: e.target.value })}
                    className="sr-only" />
                  <span className={`w-2.5 h-2.5 rounded-full ${pf.dot}`}></span>
        ...
        {fahrzeug && (
          <div>
            ...
            <div className="flex items-center gap-2 mb-2">
              <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${GAST_FARBE.dot}`}></span>
        ...
        <div>
          ...
          <textarea value={form.notiz} onChange={(e) => setForm({ ...form, notiz: e.target.value })}
            ...
            placeholder="z.B. Treffpunkt, Sonderwuensche..." />
        </div>
