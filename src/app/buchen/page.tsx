  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="grid grid-cols-3 gap-2 mb-6">
        {([
          { t: 'normal', label: 'Normale Fahrt',  desc: 'Pilot + Fahrzeug', color: 'bg-rikscha-green text-white' },
          { t: 'offen',  label: 'Offener Termin', desc: 'Pilot, Fahrzeug folgt', color: 'bg-orange-400 text-white' },
          { t: 'gast',   label: 'Gastbuchung',    desc: 'Fahrzeug, Pilot folgt', color: 'bg-cyan-600 text-white' },
        ] as const).map(({ t, label, desc, color }) => (
          <button key={t} type="button" onClick={() => setTerminTyp(t as TerminTyp)}
            className={`rounded-lg p-2 text-center border-2 transition-all ${
              terminTyp === t ? `${color} border-current shadow-md` : 'bg-white border-gray-200 text-gray-600 hover:border-gray-400'
            }`}>
      ...
      <form onSubmit={absenden} className="bg-white rounded-xl shadow p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-2">
            ...
            {!brauchtFahrzeug && (
              <label className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer border-dashed transition-colors ${
                ...
                <div><p className="font-semibold text-orange-700">Noch kein Fahrzeug</p><p className="text-xs text-gray-500">Gäste wählen später</p></div>
              </label>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          {[{ key: 'gast1', val: form.gast1, ph: 'Gast 1' }, { key: 'gast2', val: form.gast2, ph: 'Gast 2' }]
        ...
        <div>
          ...
          <textarea value={form.notiz} onChange={(e) => setForm({ ...form, notiz: e.target.value })}
            rows={2} className="w-full border rounded px-3 py-2 text-sm" placeholder="z.B. Treffpunkt..." />
        </div>
