...
const FELDER: Feld[] = [
  { schluessel: 'flyer_fahrten_text',   label: 'Fahrten – Beschreibungstext',      typ: 'textarea' },
  { schluessel: 'flyer_lotte_text',     label: 'Flotte Lotte – Beschreibung',      typ: 'textarea' },
  { schluessel: 'flyer_flitzer_text',   label: 'Flinker Flitzer – Beschreibung',   typ: 'textarea' },
  { schluessel: 'flyer_piter_text',     label: 'Jruuse Piter – Beschreibung',      typ: 'textarea' },
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F5F0E7' }}>
      ...
      <div style={{ maxWidth:720, margin:'0 auto', padding:'2rem 1.5rem 4rem' }}>
        ...
        {FELDER.map(f => (
          <div key={f.schluessel} style={{ background:'#fff', borderRadius:10, padding:'1.25rem 1.5rem', marginBottom:'1rem', boxShadow:'0 1px 4px rgba(0,0,0,0.07)' }}>
            <div style={{ fontWeight:700, fontSize:'0.82rem', color:'#2D6B1E', marginBottom:'0.6rem', letterSpacing:'0.04em' }}>{f.label}</div>
            ...
            {f.typ === 'textarea' && (
              <>
                <textarea
                  ...
                  style={{ width:'100%', border:'1.5px solid #D6CCB8', borderRadius:6, padding:'0.6rem 0.8rem', fontFamily:'inherit', fontSize:'0.88rem', resize:'vertical', outline:'none', color:'#1C1208' }}
                />
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:'0.5rem' }}>
                  <span style={{ fontSize:'0.75rem', color: speichern[f.schluessel]==='ok' ? '#16a34a' : speichern[f.schluessel]==='err' ? '#dc2626' : '#9a8a72' }}>{statusLabel(f.schluessel)}</span>
            ...
            {f.typ === 'foto' && (
              <div style={{ display:'flex', gap:'1.2rem', alignItems:'flex-start' }}>
                <div style={{ width:120, height:90, borderRadius:6, overflow:'hidden', background:'#F5F0E7', border:'1.5px dashed #D6CCB8', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
                  {werte[f.schluessel]
                ...
                <div style={{ flex:1 }}>
                  <input
                    type="file"
                    accept="image/*"
                    ref={el => { fileRefs.current[f.schluessel] = el; }}
                    style={{ display:'none' }}
