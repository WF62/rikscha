...
const FAHRZEUGE = [
  { id: 'lotte',   name: 'Flotte Lotte',   farbe: '#f59e0b', sub: 'max. 2 Gäste' },
  { id: 'flitzer', name: 'Flinker Flitzer', farbe: '#06b6d4', sub: 'Liegetandem · 1 Gast' },
  { id: 'piter',   name: 'Jruuse Piter',   farbe: '#a78bfa', sub: 'Paralleltandem · 1 Gast' },
  { id: '',        name: 'Nach Absprache', farbe: 'rgba(255,255,255,0.4)', sub: '' },
];
...
export default function GutscheinPage() {
  ...
  useEffect(() => {
    const updateScale = () => {
      const available = Math.min(window.innerWidth - 16, 740);
      setScale(Math.min(1, available / 740));
    };
    updateScale();
  ...
  const fzName = FAHRZEUGE.find(f => f.id === fahrzeug)?.name ?? 'Nach Absprache';
    ...
    <>
      <style>{`
        .g-body {
          ...
          justify-content: flex-start; min-height: calc(100vh - 120px);
          padding: 2rem 1rem 3rem; gap: 1rem; margin: -1.5rem -1rem;
        }
        ...
        .voucher { width: 740px; height: 540px; background: #F7F2E8; position: relative; overflow: hidden; box-shadow: 0 8px 40px rgba(0,0,0,0.18); }
        .voucher::before { content: ''; position: absolute; inset: 10px; border: 1.5px solid #C8881A; pointer-events: none; z-index: 10; }

        .green-panel {
          position: absolute; top: 0; left: 0; width: 52%; height: 100%;
          background: #2D6B1E; clip-path: polygon(0 0, 88% 0, 100% 100%, 0 100%);
          display: flex; flex-direction: column; align-items: flex-start;
          justify-content: space-between; padding: 40px 80px 40px 36px;
        }
        .brand-name { font-family: Palatino Linotype, Georgia, serif; font-size: 1.1rem; color: #fff; line-height: 1.25; font-weight: normal; max-width: 200px; }
        ...
        .brand-bottom { font-size: 0.68rem; color: rgba(255,255,255,0.5); line-height: 1.5; }
        ...
        .gutschein-label { font-size: 0.62rem; letter-spacing: 0.18em; text-transform: uppercase; color: #C8881A; font-weight: 700; }
        .gutschein-title { font-family: Palatino Linotype, Georgia, serif; font-size: 2.6rem; color: #1A1208; line-height: 0.95; font-weight: normal; }
        .gutschein-for { font-family: Palatino Linotype, Georgia, serif; font-size: 0.9rem; color: #4a3f30; font-style: italic; margin-bottom: 1rem; }

        .fields { display: flex; flex-direction: column; gap: 0.65rem; flex: 1; }
        .field-row { display: flex; align-items: baseline; gap: 0.5rem; border-bottom: 1px solid #C8881A; padding-bottom: 2px; }
        .field-label { font-size: 0.65rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: #2D6B1E; white-space: nowrap; flex-shrink: 0; min-width: 58px; }
        .field-value { font-family: Palatino Linotype, Georgia, serif; font-size: 0.92rem; color: #1A1208; flex: 1; min-height: 1.3em; outline: none; background: transparent; padding: 0 2px; }
        ...
        .voucher-footer { font-size: 0.58rem; color: #7a6a52; line-height: 1.5; border-top: 1px solid #e0d8cc; padding-top: 0.4rem; margin-top: 0.3rem; }
        ...
        @media print {
          ...
          .voucher-wrap { transform: none !important; margin-bottom: 0 !important; }
      ...
      <div className="g-body">
        ...
        <div className="voucher-wrap" style={{
          width: 740, height: 540,
          transformOrigin: 'top left',
          transform: scale < 1 ? `scale(${scale})` : undefined,
          marginBottom: scale < 1 ? (540 * scale - 540) : undefined,
        }}>
          <div className="voucher">
            ...
            <div className="green-panel">
              <div style={{display:'flex',flexDirection:'column',gap:'0.4rem'}}>
                ...
                <div className="brand-sub">Bornheim-Merten · seit 2018</div>
              </div>

              <div style={{display:'flex',flexDirection:'column',gap:'0.4rem'}}>
                <div style={{fontSize:'0.58rem',letterSpacing:'0.12em',textTransform:'uppercase',color:'rgba(255,255,255,0.5)',marginBottom:'0.2rem'}}>Fahrzeug wählen</div>
                {FAHRZEUGE.map(f => (
                  <div key={f.id} className={`chip${fahrzeug === f.id ? ' active' : ''}`} onClick={() => setFahrzeug(f.id)}>
                    <span className="chip-dot" style={{background: f.farbe}} />
                    <span className="chip-text" style={f.id === '' ? {color:'rgba(255,255,255,0.6)'} : undefined}>{f.name}</span>
                    {f.sub && <span className="chip-sub">{f.sub}</span>}
                  </div>
                ))}
              </div>
              ...
              <div className="brand-bottom">
                ...
                zu Olpe <span style={{color:'#C8881A'}}>(GFO)</span>
              </div>
            </div>
            ...
            <div className="g-content">
              ...
              <div className="fields">
                <div className="field-row">
                  ...
                  <div className="field-value" contentEditable suppressContentEditableWarning data-placeholder="Name des Beschenkten" />
                </div>
                <div className="field-row">
                  <span className="field-label">Fahrzeug</span>
                  <div className="field-value">{fzName}</div>
                </div>
                <div className="field-row">
                  <span className="field-label">Wunschziel</span>
                  <div className="field-value" contentEditable suppressContentEditableWarning data-placeholder="z.B. Mertener Heide, Alpakas …" />
                </div>
                <div className="field-row">
